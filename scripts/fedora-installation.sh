...
dnf -y install dnf-plugin-system-upgrade deja-dup vlc mpv mplayer mplayer-gui mencoder thunderbird\* lame gstreamer\*-plugin*\ gstreamer1-libav gstreamer1-vaapi gstreamer-ffmpeg ffmpeg pipewire gpm xvidcore empathy transmission-gtk --exclude=\*docs $NO_DEVEL $SKIP_MISSING_PACKAGE
...
## Hardware
dnf -y install beignet libclc vulkan libva-utils vdpauinfo libva-vdpau-driver libvdpau-va-gl mesa-vdpau-drivers libva-intel-driver hplip-gui pcsc-lite-devel libproxy-devel libcurl-devel

## Office
dnf -y install pandoc pandoc-pdf libreoffice-calc libreoffice-writer libreoffice-impress libreoffice-math libreoffice-base libreoffice-draw libreoffice-pdfimport kchmviewer chm2pdf gimp dia $SKIP_MISSING_PACKAGE

## Development
dnf -y groupinstall "Outils de développement et bibliothèques pour C" "Outils système"
dnf -y install gcc gcc-c++ cmake wget nasm subversion git-gui git-tools git-email openssl-devel ncurses-devel ncurses-compat-libs glibc-devel linuxdoc\* kernel-tools kernel-doc kernel-devel kernel-headers python3-sphinx rst2pdf i2c-tools minicom picocom paho-c-devel paho-c libmodbus-devel libmodbus $SKIP_MISSING_PACKAGE
